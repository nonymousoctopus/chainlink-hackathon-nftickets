...
const reviewSchema2 = yup.object({
  buyAmount: yup.number().integer().min(1),
});
...
const NFTMarketplace = () => {
 ...
 const [newItemId, setNewItemId] = useState(0);
  ...
  const readIPFS = async (currentItem, key, amountLeft, itemID) => {
    let response = await fetch(currentItem);
    let readable = await response.json();
    myMints.push({eventName: readable.name, eventImage: readable.image, eventDate: readable.date, eventTime: readable.time, eventCost: readable.price, eventAmount: amountLeft, eventLocation: readable.location, key: key, itemID: itemID});
  };
  ...
  const displayThisToken = async (_tempTokenID, itemID) => {
    let options1 = {
    ...
    await listable(itemID);
    setListModal(true);
  };

  const [itemsAvalable1, setItemsAvalable1] = useState ('');
  const [itemsAvalable2, setItemsAvalable2] = useState ('');
  //need to work out how many tickets the minter still has
  const listable = async (itemID) => {
    let options1 = {
      chain: "avalanche testnet",
      address: MARKET_CONTRACT, 
      function_name: "checkRemaining",
      abi: marketABI, 
      params: { id: itemID },
    };

    let available = await Moralis.Web3API.native.runContractFunction(options1);
    setAvailable(available);
  };

  const setAvailable = async (available) => {
    setItemsAvalable1(available);
    setItemsAvalable2(available);
  }
  ...
  const loopMe2 = async () => {
    ...
    let options1 = {
      chain: "avalanche testnet",
      address: MARKET_CONTRACT, 
      function_name: "fetchItemsOnSale",
      abi: marketABI, 
      params: { },
    };
    ...
    for (let i=0; i<result1.length; i++) {
      readIPFS(links[i],result1[i][2], result1[i][7], result1[i][0]);
    }
    setTestThis(myMints);
  ...
  const [buyHold, setBuyHold] = useState('');
  const showThisItem = async (item, itemID) => {
    ...
    setBuyHold(itemID);
    setBuyHold(newItem);
    let temp = await displayThisToken(item, itemID);
  ...
  const buyFromMarket = async (listingID, _tokenID, _amount) => {
    let options1 = {
      chain: "avalanche testnet",
      address: MARKET_CONTRACT, 
      function_name: "getPrice",
      abi: marketABI, 
      params: { nftContract: NFT_CONTRACT, tokenId: listingID },
    };
    ...
    let purchaseFee = pricePerTicket * _amount;
    await completeBuy(purchaseFee, _tokenID, _amount, selection1.name);
  };

  //This finishes the listing and pays a 20% fee to the contract
  const completeBuy = React.useCallback(async ( purchaseFee, _tokenID, amount ) => {
    let price2Bytes = web3.utils.toHex(purchaseFee);
   try {
     const data = web3.eth.abi.encodeFunctionCall({
       name: 'buyMarketItem',
       type: 'function',
       inputs: [{type: 'address', name: 'nftContract'}, {type: 'uint256', name: 'itemId'}, {type: 'uint256', name: 'amount'}, {type: 'bytes', name: 'data'}]
     }, [NFT_CONTRACT, _tokenID, amount, tokenData]) 
    ...
    <View style={styles.mainBody}>
      ...
      <View style={styles.centered}>
        ...
        <Text style={styles.formHeadingStyle}>Your event selection</Text>
            ...
            <View style={styles.eventFields}>
            <Text style={styles.eventLocation}>Start time: {selection1.time}</Text>
            <Text style={styles.eventCost}>Tickets available to buy: {itemsAvalable1}</Text>
            </View>
            ...
            <Formik
              ...
              onSubmit={(values, actions) => {
                setTokenQuantity(values.buyAmount);
                buyFromMarket(testHold, buyHold, values.buyAmount); 
                actions.resetForm();
              }}>
              {(props) => (
              <View>
                ...
                <TextInput
                  ...
                  onChangeText={props.handleChange('buyAmount')}
                  value={props.values.buyAmount}
                  onBlur={props.handleBlur('buyAmount')}/>
                <TouchableOpacity
                  style={styles.listButton}
                  activeOpacity={0.5}
                  onPress={props.handleSubmit}>
                <Text style={styles.buttonLGETextStyle}>Purchase</Text>
                </TouchableOpacity>
                </View>
                <View style={styles.formMiddleShort}>
                  <View style={styles.errorLeft}>
                    <Text style={styles.errorText}>{ props.touched.buyAmount && props.errors.buyAmount }</Text>
                  </View>
                </View>
              </View>
            )}
      ...
      <Text style={styles.formHeadingStyle}>Events on the Market</Text>  
            <TouchableOpacity
        ...
        <FlatList 
          data={testThis}
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.eventContainer}  activeOpacity={0.5} onPress={() => showThisItem(item.key, item.itemID)}>
            ...
            <Text style={styles.eventCost}>Spots left: {item.eventAmount}</Text>
            </View>
          </TouchableOpacity>
          )}
        />
